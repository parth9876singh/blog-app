...
export const sendVerificationEmail = async (email, verificationToken, userName) => {
  try {
    ...
    const { data, error } = await resend.emails.send({
      from: "CilliBlog <onboarding@resend.dev>",
      to: email,
    ...
    console.error('Exception sending verification email with Resend ❌:', error);
    return false;
  }
};
...
export const sendPasswordResetEmail = async (email, resetToken, userName) => {
  try {
    ...
    console.error('Exception sending password reset email with Resend ❌:', error);
    return false;
  }
};